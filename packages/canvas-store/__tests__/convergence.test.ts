...
// ---------------------------------------------------------------------------
// Reproduce the CanvasStore.sync() pull-push loop with arbitrary adapters.
...
describe('Sync loop convergence', () => {
  ...
  describe('non-conflicting concurrent mutations', () => {
    it('History tracks both ECS and WS changes', () => {
      ...
      const history = new HistoryAdapter({ components: [], singletons: [], commitCheckpointAfterFrames: 1 })
  ...
  // resolution. In the real CanvasStore, ECS is always first, so
