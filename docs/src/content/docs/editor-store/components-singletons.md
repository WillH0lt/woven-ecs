...
Editor Store uses custom component and singleton definitions that include metadata for syncing, history, and migrations.

* `defineComponent` -> `defineEditorComponent`
* `defineSingleton` -> `defineEditorSingleton`

```typescript
import { field } from '@woven-ecs/core';
import { defineEditorComponent } from '@woven-ecs/editor-store';
...
const Asset = defineEditorComponent({
  name: 'asset',
  sync: 'document',
...
// const Color = defineEditorComponent({
//   name: 'color',
//   sync: 'document',
// }, {
//   red: field.float32(),
